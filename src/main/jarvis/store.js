 ...
 * 上限只在这里定义一次 —— 界面侧**不写死数字**，而是通过 `session:list` 的 `titleMax`
 * 拿到这个值并设到输入框的 maxLength 上（2026-09-17 第 16 轮改）。
 * 此前界面干脆不设上限、只靠这里截断，代价是超长标题在回车那一瞬间突然变短；
 * 「两处各写一个数字会漂移」这个顾虑是对的，但解法是**把数字传过去**，不是不设防。
 * 这里仍然照旧截断：界面是显示层，防的是误操作，不是可信边界的守卫。
 */
